...
`AUTH_ADMIN_EMAILS` is a comma-separated, case-insensitive server-side
allowlist for routes in the `(admin)` group. Missing, empty, or invalid
configuration denies all administrator access. Anonymous visitors are sent to
the localized login page, while authenticated accounts outside the allowlist
receive the localized not-found response. Never expose this allowlist to client
components or browser logs. Administrator routes are always rendered on demand
so authorization is evaluated for every request.
...
Page operations use explicit route areas. Pass `--area public`, `--area user`,
or `--area admin` to direct `addpage` and `rmpage` commands and to
`addcomponent --page`. The interactive page flows expose the same area-aware
catalog. Areas select a route layout without changing the public URL, so one
logical page name cannot be reused across areas. The official areas are a
closed set rather than an extensible RBAC mechanism; see the linked command
skills for the complete contract.
...
- Fail-closed administrator routes
